...
class LuaUtils
{
	...
	public static function addAnimByIndices(obj:String, name:String, prefix:String, indices:Any = null, framerate:Float = 24, loop:Bool = false)
	{
		...
		if(obj != null && obj.animation != null)
		{
			...
			{
				var strIndices:Array<String> = Std.string(indices).trim().split(',');
			...
			if(prefix != null) obj.animation.addByIndices(name, prefix, indices, '', Std.int(framerate), loop); //std.int to shut vscode up
	...
	public static function formatVariable(tag:String)
		return trim(tag).replace(' ', '_').replace('.', '');
	...
	static function trim(s:String):String {
        ...
        while (start <= end && isSpace(s.charCodeAt(start))) {
            start++;
        }
